...
def read_model(model_data):
    ...
    for l in data.splitlines():
        ...
        if READ is True:
            ...
            c.gen.loc[g, 'PMAX'] = float(max_g)
            c.gen.loc[g, 'PMAX'] = float(pg)
            c.gen.loc[g, 'PMIN'] = float(min_g)
    ...
    for l in data.splitlines():
        ...
        if READ is True:
            ...
            c.branch.loc[branch_number, 'BR_X'] = float(r)
            c.branch.loc[branch_number, 'RATE_A'] = float(tl)
