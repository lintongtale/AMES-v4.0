        ...
        for (int d = 0; d < loadscenarios.length; d++)
            for (int hour = 0; hour < loadscenarios[d].length; hour++) {
                ...
                for (int z = 0; z < numZones; z++) {
                    sb.append(Double.toString(loadscenarios[d][hour][z]));
                    if (z < numZones - 1)
                        sb.append(", ");
                }
                sb.append("\n");
            ...
            while(s.hasNextLine()) {
                ...
                if(foundHeader){
                    ...
                    for(int z = zOffset ; z < elems.length; z++){
                        lp[z - zOffset] = Double.parseDouble(elems[z]);
                    }
    ...
    double[][][] readSplitDataScenario(File baseFile) throws Exception {
        ...
        while (d <= dmax) {
            File dayFile = new File(String.format("%s_%d.dat",
                    baseFile.getPath(), d));
            try {
                data[d - 1] = readSingleDay(dayFile);
            } catch (Exception e) {
...
//                            hour, z));
                    data[hour][z - 1] = Double.parseDouble(elems[z]);
                }
            }
        }
